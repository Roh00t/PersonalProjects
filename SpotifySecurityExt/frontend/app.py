...
class APIManager:
    ...
    def login_user(self, username, password):
        ...
        if username in failed_login_attempts and failed_login_attempts[username] >= 5:
            time.sleep(5)  # Delay login attempt
        ...
        try:
            ...
            if response.status_code == 200:
                if not verify_password(password, response_data.get("hashed_password", "")):
                    failed_login_attempts[username] = failed_login_attempts.get(username, 0) + 1
                    logger.warning(f"Failed login attempt for user: {username}")
                    return {"success": False, "message": "Invalid password."}
                failed_login_attempts[username] = 0  # Reset failed attempts on success
                return response_data
            ...
            logger.error(f"Connection error for user {username}: {str(e)}")
            return {"success": False, "message": f"Failed to connect to the server: {str(e)}"}
